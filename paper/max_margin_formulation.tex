...
We approach this problem by formulating our trajectory transfer problem as an abstract MDP. 
In this MDP, 
...
\end{equation*}
where \statesubsup{i}{(j)}
is a state and \actionsubsup{i}{(j)} is the action taken in that state, described in
Section~\ref{subsec:form_mdp}. Each labeled sequence \labelsub{j} corresponds
to a single complete task execution, ordered in the chronological sequence of
states and actions taken in those states: \statesubsup{i}{(j)} should
be the result of taking action \actionsubsup{i-1}{(j)} in state \statesubsup{i-1}{(j)},
and the last state \statesubsup{\nsub{j}}{(j)} should be a goal state.
