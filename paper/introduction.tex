...
%%PA: I might cut out the above as it's pretty generic (and
...
%%PA: might cut the "current approaches ... " as it could put a
%%reviewer on the defensive

%%PA: there seems a bit of a jump here, going from the challenges in
%%robotic manipulation of deformable object manipulation to learning
%%from demonstrations; how about more or less just skipping the
%%paragraph that follows and going straight into describing the recent
%%work that has presented a learning from demonstrations approach that
%%is suitable for deformable objects.
...
%%PA: there are some jumps in reasoning here; one aspect is to
%%consider more than one demonstration; another aspect is that
%%demonstrations consist of segments;  I'd introduce the concept of
%%segments above already (e.g., explaining a demonstrated knot tie shown in Figure
%%1 consists of 4 segments?);  it'll also be important to be
%%consistent in terminology, right now the intro talks about
%%sequencing trajectory generalizations, do we want to use the word
...
%%PA: it might be good to be specific here and give some examples;
%%e.g., when generalizing from a demonstration segment that barely
%%grasps the rope the resulting warped trajectory might fail to grasp
%%the rope, or a placement that only ever so barely induces a crossing
%%might result in a warped trajectory that doesn't achieve the
%%crossing; you might know better examples
Furthermore, the existence of poor or brittle demonstrations can very negatively 
effect performance.

%%PA: the original paper presenting this approach(cite isrr2013)
...
%%should lead the preceding paragraph with something like this
%%sentence --- then go into why that might not be the right thing;
%% 
...
%%PA: how about being explicit here and leading with something like:
%%"The contribution of this paper are: (i) ... (ii) ... with one
%%paragraph per contribution, and every contribution having a (i)
%%leading off the paragraph; it'd be good to also match this up with
%%how the paper is organized in terms of sections/subsections; such
%%that it's possible to flip to the (sub)section with a particular contribution

%%PA: paragraph below is just a bit too general; I'd make it more directly
%%fit with what we actually do (rather than trying to generalize it
%%for this paragraph)
...
%%PA: how about leading off this paragraph differently: "While our running example
%%and all our experiments
%%consider the problem of knot tying, the presented approach is not
%%specific to knot tying.  It more generally applies to ... "
...
%%PA: last contribution paragraph should describe the experimental
%%results;  Say we contributed a benchmark (or used the existing
%%benchmark from ISRR 2013, not sure what's true) which is made
%%available at the following url: ... .   Say that the ISRR paper
%%achieves x % performance, the proposed approach achieves y %
%%performance.   


%%PA: paragraph below is redundant given what we will already have above;
%%let's just cut it?
